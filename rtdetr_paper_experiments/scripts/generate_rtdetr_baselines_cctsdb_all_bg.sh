...
LOG_FILE="${LOG_FILE:-rtdetr_paper_experiments/logs/generate_rtdetr_baselines_cctsdb_all_$(date +%Y%m%d_%H%M%S).log}"

run_one() {
  local attack="$1"
  local output="rtdetr_paper_experiments/adv_outputs/${attack}/cctsdb_whitebox"
  local args=(
    --attack "${attack}"
    --data "${DATA}"
    --weights "${WEIGHTS}"
...
echo "Started all RT-DETR white-box baseline generation on CCTSDB."
echo "PID: $!"
