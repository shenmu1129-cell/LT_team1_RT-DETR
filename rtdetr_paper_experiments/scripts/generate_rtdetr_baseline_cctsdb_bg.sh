...
GPU_ID="${GPU_ID:-5}"
ATTACK="${ATTACK:-tog}"  # tog, daedalus/dae, osfd/osfp
DATA="${DATA:-data/cctsdb.yaml}"
WEIGHTS="${WEIGHTS:-runs/detect/outputs/rtdetr_cctsdb/weights/best.pt}"
OUTPUT="${OUTPUT:-rtdetr_paper_experiments/adv_outputs/${ATTACK}/cctsdb_whitebox}"
IMGSZ="${IMGSZ:-960}"
BATCH="${BATCH:-2}"
STEPS="${STEPS:-10}"
EPS="${EPS:-0.031372549}"
...
LOG_FILE="${LOG_FILE:-rtdetr_paper_experiments/logs/generate_${ATTACK}_rtdetr_cctsdb_$(date +%Y%m%d_%H%M%S).log}"
...
echo "Started RT-DETR white-box baseline generation on CCTSDB."
echo "PID: $!"
echo "Attack: ${ATTACK}"
echo "GPU_ID: ${GPU_ID}"
echo "Output: ${OUTPUT}"
